...
Azure Function project that receives email content, researches additional context with OpenAI web search, rewrites it with OpenAI, and creates a WordPress draft with categories, tags, ACF lead text, sources, and optionally a featured image generated through the OpenAI API.
...
Example OpenAI web search settings:
...
## Notes
  ...
  * OpenAI web search can be enabled with ENABLE_OPENAI_WEB_SEARCH=true
  * OPENAI_WEB_SEARCH_CONTEXT_SIZE controls how much web context is used. Supported values are low, medium, and high.
  * OPENAI_WEB_SEARCH_BLOCKED_DOMAINS can be used to exclude domains from web search results.
